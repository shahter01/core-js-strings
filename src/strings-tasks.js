...
function isPalindrome(str) {
  const lowerStr = str.toLowerCase().replaceAll(/[?!, ]/gi, '');
  let result = '';
  for (let i = lowerStr.length - 1; i >= 0; i -= 1) {
    if (lowerStr[i] !== '!' || lowerStr[i] !== '?' || lowerStr[i] !== ',') {
      result += lowerStr[i];
    }
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    result += arrStr[i].split('').reverse().join('').concat(' ');
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < arrStr.length; i += 1) {
    if (arrStr[i] === arrStr[i].toUpperCase()) {
      result += arrStr[i].toLowerCase();
    } else {
      result += arrStr[i].toUpperCase();
    }
  }
  return result;
}
...
function extractNameFromTemplate(value) {
  return value.split('').slice(7).slice(0, -1).join('');
}
...
function encodeToRot13(str) {
  ...
  const cryptAlpha = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm ?!';
  let encoded = '';
  for (let i = 0; i < str.length; i += 1) {
    const index = alpha.indexOf(str[i]);
    encoded += cryptAlpha[index];
  }
  return encoded;
}
